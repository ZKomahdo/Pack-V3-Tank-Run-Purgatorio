SurvivorPurchaseFunc_Quick(client)
{
	if (PurchaseItem[client] == 0)	// Free Grenades
	{
		if (GrenadeCount[client] < 1) PrintToChat(client, "%s \x01No Grenades Left in your Supply!", ERROR_INFO);
		else
		{
			if (StrEqual(ItemName[client], "pipe_bomb") && Gr[client][2] >= GetConVarInt(GrenPipeLevel))
			{
				L4D_RemoveWeaponSlot(client, L4DWeaponSlot_Grenade);
				GrenadeCount[client]--;
				ExecCheatCommand(client, "give", "pipe_bomb");
			}
			else if (StrEqual(ItemName[client], "vomitjar") && Gr[client][2] >= GetConVarInt(GrenJarLevel))
			{
				L4D_RemoveWeaponSlot(client, L4DWeaponSlot_Grenade);
				GrenadeCount[client]--;
				ExecCheatCommand(client, "give", "vomitjar");
			}
			else if (StrEqual(ItemName[client], "molotov") && Gr[client][2] >= GetConVarInt(GrenMolLevel))
			{
				L4D_RemoveWeaponSlot(client, L4DWeaponSlot_Grenade);
				GrenadeCount[client]--;
				ExecCheatCommand(client, "give", "molotov");
			}
			else PrintToChat(client, "%s \x01Your Grenade level is not high enough to purchase this item.", ERROR_INFO);
		}
	}
}
...
SurvivorPurchaseFunc(client)
{
	...
	{
		// IF No Increment (0) or Incrementing costs is enabled (1)
	...
	{
		// If level-based costs is enabled.
		if (PurchaseItem[client] == 1) ItemCost[client] = Ph[client][2] * GetConVarFloat(Tier2CostByLevel);
		else if (PurchaseItem[client] == 2) ItemCost[client] = It[client][2] * GetConVarFloat(HealthItemCostByLevel);
		else if (PurchaseItem[client] == 3) ItemCost[client] = Ph[client][2] * GetConVarFloat(PersonalAbilitiesCostByLevel);
		else if (PurchaseItem[client] == 4) ItemCost[client] = Ph[client][2] * GetConVarFloat(WeaponUpgradeCostByLevel);
	}
	...
	else if (PurchaseItem[client] == 2)
	{
		...
		{
			ExecCheatCommand(client, "give", ItemName[client]);
			if (StrEqual(ItemName[client], "health", false))
			{
				SetEntityHealth(client, 100 + Ph[client][2] + It[client][2]);
			}
		}
	}
	...
	{
		if (GetConVarInt(CostEvaluation) == 1)
		{
			// IF CostEvaluation == 1, Purchases are incrementing.
