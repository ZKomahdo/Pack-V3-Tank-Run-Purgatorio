...
public Action:Event_PlayerShoved(Handle:event, String:event_name[], bool:dontBroadcast)
{
	...
	if (IsHuman(client) && GetClientTeam(client) == 3 && IsHuman(attacker) && GetClientTeam(attacker) == 2)
	{
		if (ClassBoomer[client] && SuperUpgradeBoomer[client] && IsPlayerAlive(client))
		{
			SetEntityHealth(client, 1);
			IgniteEntity(client, 1.0);
		}
		else if (Me[attacker][2] >= GetConVarInt(ShoveBounceLevel) && IsPlayerAlive(client))
		{
			MeleeShoveBounce(attacker, client);
		}
	}
}
...
public Action:Event_PlayerDeath(Handle:event, String:event_name[], bool:dontBroadcast)
{
	...
	if (!IsClientIndexOutOfRange(victim) && IsClientInGame(victim) && GetClientTeam(victim) == 3)
	{
		...
		// Start the timer that will set a players class when they hit ghost mode.
		if (!IsFakeClient(victim)) CreateTimer(0.1, Timer_IsPlayerGhost, victim, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
	}
	if (!IsClientIndexOutOfRange(attacker) && IsClientInGame(attacker) && !IsFakeClient(attacker) && GetClientTeam(attacker) == 2)
	{
...
MeleeShoveBounce(attacker, victim)
{
	new Float:vel[3];
	vel[0] = GetEntPropFloat(victim, Prop_Send, "m_vecVelocity[0]");
...
InfectedPlayerHasDied(infected)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		if (AssistHurtInfected[i][infected] > 0.0)
		{
			...
			Ph[i][0] += RoundToFloor(xp_reward * GetConVarFloat(EventMoreXPFriday));
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		*/
		Pi[i][0] += RoundToFloor(PistolDamage[i][infected] * GetConVarFloat(EventMoreXPFriday));
		Me[i][0] += RoundToFloor(MeleeDamage[i][infected] * GetConVarFloat(EventMoreXPFriday));
		Uz[i][0] += RoundToFloor(UziDamage[i][infected] * GetConVarFloat(EventMoreXPFriday));
		Sh[i][0] += RoundToFloor(ShotgunDamage[i][infected] * GetConVarFloat(EventMoreXPFriday));
		Sn[i][0] += RoundToFloor(SniperDamage[i][infected] * GetConVarFloat(EventMoreXPFriday));
		Ri[i][0] += RoundToFloor(RifleDamage[i][infected] * GetConVarFloat(EventMoreXPFriday));
